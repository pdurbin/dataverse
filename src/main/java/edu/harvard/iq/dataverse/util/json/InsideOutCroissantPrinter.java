...
public class InsideOutCroissantPrinter {

    private static final Logger logger = Logger.getLogger(InsideOutCroissantPrinter.class.getCanonicalName());

    private InsideOutCroissantPrinter() {
    }

    public static String get(Dataset dataset) {
        if (dataset == null) {
    ...
    public static String get(ExportDataProvider dataProvider) throws ExportException {
        // System.out.println("export data provider");
        // return "export data provider";
        try {
            ...
            // if (true) {
            //     // job.add("foo", datasetJson);
            //     // TODO: How do we know which metadata blocks contain rubrics?
            //     String out = job.build().toString();
            //     return out;
            // }

            JsonObject datasetORE = dataProvider.getDatasetORE();
            // if (true) {
            //     job.add("oreTest", datasetORE);
            //     String out = job.build().toString();
            //     return out;
            // }
            System.out.println("BEGIN");
            JsonUtil.prettyPrint(datasetORE);
            ...
            // We don't escape DatasetSchemaDotOrg fields like creator, description, etc.
            // because
            ...
            /**
             * For "version", we are knowingly sending "1.0" rather than "1.0.0", even
             * though
             * MAJOR.MINOR.PATCH is recommended by the Croissant spec. We are aware that the
             * Croissant validator throws a warning for anything other than
             * MAJOR.MINOR.PATCH. See
            ...
            /**
             * We have been told that it's fine and appropriate to put the citation to the
             * dataset
             * itself into "citeAs". However, the spec says "citeAs" is "A citation for a
             * publication that describes the dataset" so we have asked for clarification
             * here:
            ...
            for (JsonValue jsonValue : datasetFileDetails) {
                ...
                /**
                 * When there is an originalFileName, it means that the file has gone through
                 * ingest
                 ...
                 * https://github.com/mlcommons/croissant/issues/641 . Should we? Is there
                 * interest
                 * in this? And would we duplicate all the cr:RecordSet entries (columns) with
                 * each
                 * additional format? Probably not as it would be the same.
                 */
                String filename = StringEscapeUtils.escapeHtml4(
                 ...
                 * (JsonNumber) to pass the Croissant validator and comply with the spec. We
                 * don't
                ...
                String contentUrl = oreFiles.getJsonObject(positionInOre).getString("schema:sameAs");
                 ...
                 * @id (path/to/file.txt).
                 *     <p>
                 *     It's suboptimal that the directoryLabel isn't already included in
                ...
                String directoryLabel = oreFiles.getJsonObject(positionInOre)
                ...
                for (JsonValue dataTableValue : dataTables) {
                    ...
                    for (JsonValue dataVariableValue : dataVariables) {
                        ...
                        String variableDescription = StringEscapeUtils.escapeHtml4(
                                dataVariableObject.getString("label", ""));
                        String variableFormatType = dataVariableObject.getString("variableFormatType");
                        String variableIntervalType = dataVariableObject.getString("variableIntervalType");
    ...
    private static String getNumericType(String variableIntervalType) {
        /**
         * According to DataVariable.java in Dataverse, the four possibilities are:
         * discrete, contin
