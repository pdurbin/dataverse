    ...
    public Response getRsync(@PathParam("id") String id,
        ...
        try {
            ...
            if (StringUtils.isNotEmpty(uploadMethods) && uploadMethods.toUpperCase().contains("RSYNC")) {
                LOGGER.info("sleep requested: "+ sleep);
...
//    }
    ...
    /**
     * @todo Revisit auth for this endpoint. User authentication is handled in
     * two ways 1) the endpoint requires a dataverse superuser api key 2) the
     * depositing dataverse user (indicated by userId in the json request body)
     * must have Permission.AddDataset on the dataset's parent dataverse
     */
    @POST
    ...
    public Response receiveChecksumValidationResults(JsonObject result) {
        ...
        LOGGER.info("receiveChecksumValidationResults received following JSON: " + result);
        ...
        try {
            ...
            /** @todo Revisit auth for this endpoint. */
            // only superusers should run this
//            User superUser = findUserOrDie();
//            if (!superUser.isSuperuser()) {
...
//            String userid = result.getString("userId").replace("@", "");
            JsonNumber useridJsonNumber = result.getJsonNumber("userId");
//            JsonValue.ValueType foo = userid.getValueType();
            Long userid = useridJsonNumber.longValueExact();
            JsonNumber datasetIdJsonNumber = result.getJsonNumber("datasetId");
            Long datasetId = datasetIdJsonNumber.longValueExact();
...
//            int datasetid = result.getInt("datasetId");
            ...
            Dataset dataset = findDatasetOrDie(Long.toString(datasetId));
...
//            depositingUser = authSvc.getAuthenticatedUser(userid);
            depositingUser = authSvc.findByID(userid);
            if (depositingUser == null) {
                return error(Response.Status.BAD_REQUEST, "Cannot find a user based on ID " + userid);
            }
            LOGGER.log(Level.INFO, "Depositing user: " + depositingUser.getIdentifier());
            Dataverse dv = dataset.getOwner();
            List<AuthenticatedUser> userList = permissionSvc.getUsersWithPermissionOn(Permission.AddDataset, dv);
            for (AuthenticatedUser au : userList) {
                LOGGER.log(Level.INFO, "User: " + au.getIdentifier());
            }
            if (!permissionSvc.userOn(depositingUser, dv).has(Permission.AddDataset)) {
                LOGGER.log(Level.SEVERE,
            ...
            if (passed.equals(status)) {
                ...
                /**
                 * @todo Switch to jo.start once
                 * https://github.com/IQSS/dataverse/pull/3497 is merged.
                 */
